...
public class SandboxArreglos
{
    ...
    //Investigando un poco, es mala idea usar Iterator cuando hablamos de arreglos, por lo que use for siempre
    
    
    public int[] getCopiaEnteros( )
    
    {
    	//Creo el nuevo arreglo con la longitud del arreglo original
    ...
    {
        //Lo mismo que arriba pero con cadenas
    	String[] copia = new String[ arregloCadenas.length];
    	
    	//La condicion despues el  ; es de 0 a len del String -1, tal como en python. Y luego cambio posicion por posicion. 
    ...
    public void reiniciarArregloCadenas( Object[] objetos )
    {
        ...
        {
        //METODO .TOSTRING PARA CONVERTIRLO A STRING. OBJETO -> CADENA
    ...
    {
    	//LEXICOGRAFICAMENTE = COMO APARECEN EN EL DICCIONARIO
    ...
    {
        HashMap<Integer, Integer> mapaResultado = new HashMap<Integer, Integer>();
        
        for (int i = 0; i < arregloEnteros.length; i++)
        {
        	int valorAux = arregloEnteros[i];
        	
        	if (mapaResultado.containsKey(valorAux))
        	{
        		mapaResultado.put(valorAux, mapaResultado.get(valorAux)+1);
        
        	}
        	
        	else
        	{
        		mapaResultado.put(valorAux, 1);
        	}
        }
    ...
    public boolean mismosEnteros( int[] otroArreglo )
    {
        ...
        // A pesar de que quería usar una función que construí para usarla acá, no puedo porque eso modifica el arreglo principal, y esa no es la idea. 
        
        
        for (int i = 0; i < copia1.length; i++)
        {
        	for  (int j = 0; j < copia1.length - 1; j++)
        	{
        		if (copia1 [j] > copia1[j+1])
        		{
        			int temporal = copia1[j];
        			copia1[j] = copia1[j+1];
        			copia1[j+1] = temporal;
        		}
        	}
        ...
        {
            for ( int j = 0; j < copia2.length - 1; j++ )
            {
                if ( copia2[j] > copia2[j + 1] )
                {
                    int temporal = copia2[j];
                    copia2[j] = copia2[j + 1];
                    copia2[j + 1] = temporal;
                }
            }
