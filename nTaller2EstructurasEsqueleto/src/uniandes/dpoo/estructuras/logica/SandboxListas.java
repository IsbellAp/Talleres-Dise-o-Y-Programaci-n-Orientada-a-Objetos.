...
public class SandboxListas
{
    ...
    public void eliminarEntero( int valor )
    {
    	...
    	// Aun no entiendo muy bien la diferencia entre el Iterator con el while o for
    	//Entonces lo estoy usando en algunas funciones como para probar, pero de igual forma me siento mas comoda con el for 
    ...
    public void eliminarCadena( String cadena )
    {
    	...
    	//Aquí yo debo usar equals
    	
    	 Iterator<String> a = listaCadenas.iterator();

    	    while (a.hasNext())
    	    {
    	        if (a.next().equals(cadena))
    	        {
    	            a.remove();
    	        }
    	    }
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	...
    	{
    	    listaEnteros.add((int) valores[i]); //el int para dejar solo el entero
    	}    	
    ...
    public int contarEnterosRepetidos( )
    {
    	...
    	Map <Integer,Integer> auxconteo = new HashMap <Integer, Integer>();
    	
    	for (int i = 0; i < listaEnteros.size(); i++)
    	{
    		int valor = listaEnteros.get(i);
    		
    		if (auxconteo.containsKey(valor))
    		{
    			auxconteo.put(valor, auxconteo.get(valor)+1);
    		}
    		else
    		{
    			auxconteo.put(valor, 1);
    		}
    			
    	}
    	
    	int resultado = 0;
    	for (Integer llave : auxconteo.keySet())
    	{
    		if (auxconteo.get(llave) > 1)
    		{
    			resultado++;
    		}
    	}
    ...
    public void generarEnteros( int cantidad, int minimo, int maximo )
    {
    	...
    	{
    	    int a = minimo + (int)(Math.random() * (maximo - minimo + 1));
    	    listaEnteros.add(a);
    	}
    }
